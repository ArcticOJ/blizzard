...
type DRPCIglooClient interface {
	...
	Health(ctx context.Context, in *emptypb.Empty) (*IglooHealth, error)
...
func (c *drpcIglooClient) Health(ctx context.Context, in *emptypb.Empty) (*IglooHealth, error) {
	out := new(IglooHealth)
	err := c.cc.Invoke(ctx, "/arctic.Igloo/Health", drpcEncoding_File_igloo_proto{}, in, out)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type DRPCIglooServer interface {
	Health(context.Context, *emptypb.Empty) (*IglooHealth, error)
	Judge(*File, DRPCIgloo_JudgeStream) error
}

type DRPCIglooUnimplementedServer struct{}

func (s *DRPCIglooUnimplementedServer) Health(context.Context, *emptypb.Empty) (*IglooHealth, error) {
...
func (DRPCIglooDescription) NumMethods() int { return 2 }

func (DRPCIglooDescription) Method(n int) (string, drpc.Encoding, drpc.Receiver, interface{}, bool) {
	switch n {
	case 0:
		return "/arctic.Igloo/Health", drpcEncoding_File_igloo_proto{},
...
type DRPCIgloo_HealthStream interface {
	drpc.Stream
	SendAndClose(*IglooHealth) error
}

type drpcIgloo_HealthStream struct {
	drpc.Stream
}

func (x *drpcIgloo_HealthStream) SendAndClose(m *IglooHealth) error {
